...
struct llama_init_result llama_init_from_gpt_params(gpt_params & params) {
    ...
    if (params.warmup) {
        ...
        if (llama_model_has_decoder(model)) {
            llama_batch origin_batch = llama_batch_get_one(tmp.data(), std::min(tmp.size(), (size_t) params.n_batch), 0, 0);
            LOG("origin_batch: %d", origin_batch.type);
            llama_decode(lctx, origin_batch);
        }
